...
<head>
    ...
    <title>Reset Sandi | Sistem Inventaris</title>
    ...
    <div class="flex min-h-screen items-center justify-center p-4 lg:p-8">
        <div class="w-full max-w-md">
            ...
            <div class="mb-10 text-center">
                ...
                <h1 class="text-2xl font-bold tracking-tight text-white">Buat Sandi Baru</h1>
                <p class="mt-2 text-sm text-slate-400">Pastikan sandi baru Anda kuat dan mudah diingat</p>
            </div>
            ...
            <div class="overflow-hidden rounded-3xl border border-slate-800 bg-[#111827] shadow-2xl">
                <div class="p-8 lg:p-10">
                    ...
                    <form action="{{ route('password.update') }}" method="POST" class="space-y-6"
                          x-data="{ loading: false, showPass: false, showConfirm: false, password: '', strength: 0 }"
                          @submit="loading = true"
                          x-init="$watch('password', val => {
                              let s = 0;
                              if (val.length >= 8) s++;
                              if (/[A-Z]/.test(val)) s++;
                              if (/[0-9]/.test(val)) s++;
                        ...
                        <!-- Input Password Baru -->
                        <div>
                            <label for="password" class="mb-2 block text-sm font-medium text-slate-300">Kata Sandi Baru</label>
                            <div class="relative">
                                ...
                                <button type="button" @click="showPass = !showPass"
                                    class="absolute inset-y-0 right-0 flex items-center pr-3 text-slate-500 hover:text-blue-400">
                                    <svg x-show="!showPass" xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                                    ...
                                    <svg x-show="showPass" x-cloak xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                            ...
                            <div x-show="password.length > 0" x-cloak class="mt-2">
                                <div class="flex gap-1">
                                    <div class="h-1.5 flex-1 rounded-full transition-all duration-300"
                                         :class="strength >= 1 ? (strength === 1 ? 'bg-red-500' : strength === 2 ? 'bg-orange-500' : strength === 3 ? 'bg-yellow-500' : 'bg-emerald-500') : 'bg-slate-700'"></div>
                                    <div class="h-1.5 flex-1 rounded-full transition-all duration-300"
                                         :class="strength >= 2 ? (strength === 2 ? 'bg-orange-500' : strength === 3 ? 'bg-yellow-500' : 'bg-emerald-500') : 'bg-slate-700'"></div>
                                    <div class="h-1.5 flex-1 rounded-full transition-all duration-300"
                                         :class="strength >= 3 ? (strength === 3 ? 'bg-yellow-500' : 'bg-emerald-500') : 'bg-slate-700'"></div>
                                    <div class="h-1.5 flex-1 rounded-full transition-all duration-300"
                                         :class="strength >= 4 ? 'bg-emerald-500' : 'bg-slate-700'"></div>
                                </div>
                                <p class="mt-1 text-xs"
                                   :class="{
                                    ...
                                    <span x-text="['', 'Lemah', 'Cukup', 'Kuat', 'Sangat Kuat'][strength]"></span>
                                </p>
                            </div>
                        </div>
                        ...
                        <div>
                            <label for="password_confirmation" class="mb-2 block text-sm font-medium text-slate-300">Konfirmasi Kata Sandi</label>
                            <div class="relative">
                                <span class="absolute inset-y-0 left-0 flex items-center pl-3.5 text-slate-500">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                                        <path fill-rule="evenodd" d="M5 9V7a5 5 0 0110 0v2a2 2 0 012 2v5a2 2 0 01-2 2H5a2 2 0 01-2-2v-5a2 2 0 012-2zm8-2v2H7V7a3 3 0 016 0z" clip-rule="evenodd" />
                                ...
                                <button type="button" @click="showConfirm = !showConfirm"
                                    ...
                                    <svg x-show="showConfirm" x-cloak xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                                        <path fill-rule="evenodd" d="M3.707 2.293a1 1 0 00-1.414 1.414l14 14a1 1 0 001.414-1.414l-1.473-1.473A10.014 10.014 0 0019.542 10C18.268 5.943 14.477 3 10 3a9.991 9.991 0 00-4.512 1.074l-1.78-1.781zm4.261 4.26l1.514 1.515a2.003 2.003 0 012.45 2.45l1.514 1.514a4 4 0 00-5.478-5.478z" clip-rule="evenodd" />
                                        <path d="M12.454 16.697L9.75 13.992a4 4 0 01-3.742-3.741L2.335 6.578A9.98 9.98 0 00.458 10c1.274 4.057 5.065 7 9.542 7 .847 0 1.669-.105 2.454-.303z" />
                                    </svg>
                                </button>
                        ...
                        <button type="submit" :disabled="loading"
                            ...
                            <span x-show="!loading">SIMPAN SANDI BARU</span>
                ...
                <div class="border-t border-slate-800 bg-slate-900/50 px-8 py-4 text-center">
                    <p class="text-xs text-slate-500 italic">Setelah berhasil, Anda akan diarahkan ke halaman masuk</p>
                </div>
            </div>
